...
const ANALITICOS_INFO: { key: keyof Analiticos; label: string; icon: string; cor: string }[] = [
  { key: 'queda_leito',    label: 'Queda do Leito',    icon: '', cor: '#EF4444' },
  { key: 'queda_pe',       label: 'Queda em P',       icon: '', cor: '#F97316' },
  ...
  { key: 'linha_contagem', label: 'Linha de Contagem', icon: '', cor: '#10B981' },
  { key: 'habitos',        label: 'Hbitos',           icon: '', cor: '#F59E0B' },
]
...
function CameraPlayer({ camera, cameraAoVivoId, setCameraAoVivoId }: { camera: Camera, cameraAoVivoId: string | null, setCameraAoVivoId: (id: string | null) => void }) {
  ...
  async function onMouseUp(e: React.MouseEvent) {
    ...
    } catch { setErro('Erro ao salvar regio') }
  }
      ...
      <div className="p-4">
        ...
        {abaAtiva === 'regioes' && (
          <div className="bg-gray-900 rounded-lg p-3">
            <p className="text-gray-400 text-xs mb-2 font-bold">REGIES DE IA  clique e arraste na imagem</p>
            {!carregouAnaliticos ? (
              <p className="text-gray-500 text-xs">Carregando...</p>
            ) : tiposLiberados.length === 0 ? (
              <p className="text-yellow-500 text-xs">Ative ao menos um analtico com regio no painel </p>
            ) : (
              <>
                ...
                {tiposLiberados.includes('linha') && (
                  <div className="mb-2 p-2 rounded-lg border border-green-700 bg-green-900/20">
                    <p className="text-green-400 text-xs font-bold"> Linha de Contagem</p>
        ...
        {abaAtiva === 'analiticos' && (
          <div className="bg-gray-900 rounded-lg p-3">
            <div className="flex items-center justify-between mb-3">
              <p className="text-gray-400 text-xs font-bold">ANALTICOS DE IA</p>
            ...
            <p className="text-gray-600 text-xs mt-3"> = possui regio de IA configurvel</p>
          </div>
        )}
      </div>
    </div>
    ...
    <main className="min-h-screen bg-gray-950 text-white p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-blue-400">Cmeras ao Vivo</h1>
            <p className="text-gray-400 mt-1">{cameras.length} cmera{cameras.length !== 1 ? 's' : ''} cadastrada{cameras.length !== 1 ? 's' : ''}</p>
          </div>
        ...
        {carregando ? (
          ...
          <div className="text-center py-20 text-gray-500">
            ...
            <p className="text-xl">Nenhuma cmera cadastrada</p>
            <Link href="/" className="mt-4 inline-block bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded-lg font-bold transition">+ Adicionar cmera</Link>
          </div>
        ) : (
